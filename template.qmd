...
temp<-read_tsv(here::here('data','seeds_dataset.txt'),
                     col_names = FALSE) |> 
  setNames(var_names) #adds column names to df
```
...
ggplot(seeds_df, aes(x = a, y = asym, color = variety, shape = variety)) +
  geom_point() + 
  geom_smooth() +
  labs(x = "Area", 
       y = "Asymmetry") +
...
```{r}
ggplot(seeds_df, aes(x = l_g, y = w_k, color = variety, shape = variety)) +
  geom_point() + 
  geom_smooth() +
  labs(x = "length of kernel groove", 
       y = "width of kernel") +
  theme_bw()
```
...
seeds_complete <- seeds_df |>
 ...
 ## the complete with scaled data

#can only scale as.numeric observations 
...
```{r}
#| fig-cap: we would select 3 groups to insiate a kmeans cluster beasuce the kink in the grpah is most pronouced at t3 clusters
...
```{r}
#| tbl-cap: Contiency table for the grouping of kmeans and the variables of kernles
...
!. slice out top 20 emitter

2. drop na (note" we don't have any)

3. select only the numeric
...
rownames(wb_scaled) <- wb_ghg_20$name #adds row names 
...
combine <- p_complete + p_single + plot_layout(ncol = 2)
